...
namespace TicketManager.WebAPI.Services.NotificationHandlers
{
    public abstract class QueryStoreSyncNotificationHandlerBase
    {
        ...
        protected async Task<IEnumerable<string>> GetUpdatedInvolvedUsersAsync(EventsContext context, long ticketCreatedEventId, Ticket ticket, CancellationToken cancellationToken)
        {
            var involvement = ticket.Involvement;

            // TODO: Consider comment events
            // TODO: Return latest change ids
            var addInvolvements1 = await context.TicketAssignedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownAssignmentChangeId).AsEventBase()
                .Concat(context.TicketDescriptionChangedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownDescriptionChangeId).AsEventBase())
                .Concat(context.TicketLinkChangedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownLinkChangeId).AsEventBase())
                .Concat(context.TicketPriorityChangedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownPriorityChangeId).AsEventBase())
                .Concat(context.TicketStatusChangedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownStatusChangeId).AsEventBase())
                .Concat(context.TicketTagChangedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownTagChangeId).AsEventBase())
                .Concat(context.TicketTitleChangedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownTitleChangeId).AsEventBase())
                .Concat(context.TicketTypeChangedEvents.OfTicket(ticketCreatedEventId).After(involvement.LastKnownTypeChangeId).AsEventBase())
                .Select(evt => new
                {
                    User = evt.CausedBy,
                    evt.UtcDateRecorded,
            ...
            var addInvolvements2 = await context
                .TicketAssignedEvents
                .OfTicket(ticketCreatedEventId)
                .After(involvement.LastKnownAssignmentChangeId)
            ...
            var removeInvolvements = await context
                .TicketUserInvolvementCancelledEvents
                .OfTicket(ticketCreatedEventId)
                .After(involvement.LastKnownCancelInvolvementId)
                .Select(evt => new
                {
                    User = evt.CausedBy,
                    evt.UtcDateRecorded,
            ...
            var result = new HashSet<string>(ticket.Involvement.InvoledUsersSet);

            foreach (var involvementChange in involvementChanges)
            {
                if (involvementChange.AddInvolvement)
                {
                    result.Add(involvementChange.User);
                }
                else
                {
                    result.Remove(involvementChange.User);
                }
            }
