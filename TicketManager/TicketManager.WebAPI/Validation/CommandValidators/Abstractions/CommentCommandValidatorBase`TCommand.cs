...
namespace TicketManager.WebAPI.Validation.CommandValidators.Abstractions
{
        ...
        where TCommand : ICommentCommand
    {
        ...
        {
            context.RootContextData[ValidationContextKeys.FoundCommentIdsContextDataKey] = await FindExistingReferencedCommentIdsAsync(context.InstanceToValidate, cancellationToken).ConfigureAwait(false);
        ...
        /// </returns>
        protected virtual async Task<ISet<long>> FindExistingReferencedCommentIdsAsync(TCommand command, CancellationToken cancellationToken)
        {
